//**************  begin SummerSports SCHEME  *************************
var submenuOffsetX = -1;

//*********  Init functions *********************
function init_SummerSports()
{
}
//**************  end (Init functions) ****************

function change_color_SummerSports(id, type, last, first, arrow)	{
...
function change_color_root_SummerSports(id, type, selected)	{
...
function change_root_SummerSports(id, type)	{
	tdid = "td_" + id; td2id = "td2_" + id;
	if(document.getElementById(tdid))
		change_color_root_SummerSports(id, type);
	else	{
		last = cat_ids[id][0];
		first = cat_ids[id][1];
		arrow = cat_ids[id][2];
		change_color_SummerSports(id, type, last, first, arrow);
	}
}
function change_parents_color_SummerSports(cat_id, type)	{
	if(cat_id != 0)	{
		var content = " ";
		for (var x = 0; x < parents[cat_id].length; x++)	{
			current_id = parents[cat_id][x];
			if(current_id != cat_id)	{
				change_root_SummerSports(current_id, type);
			}
		}
	}
}

//**************  end SummerSports SCHEME  *************************

//********************************* Images init  *********************
function InitMenuImages_SummerSports() {
...
function CategoryBody_SummerSports(id, href, caption, cat_id, first)
{
...
function SubCategoryBody_SummerSports(id, href, caption, show_arrow, iconUrl, showIcon, cat_id, is_first, is_last, arrow)
{
	obj = document.getElementById(id);
    if ( !obj ) return;
