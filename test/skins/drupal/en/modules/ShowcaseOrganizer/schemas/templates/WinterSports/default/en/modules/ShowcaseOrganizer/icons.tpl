...
<tr>
	<td FOREACH="row,product" align="center" width="{getPercents(config.ShowcaseOrganizer.so_columns)}%" valign="top">
    	<span IF="product&config.ShowcaseOrganizer.so_show_price"><FONT class="ProductPriceTitle">Price: </FONT><FONT class="ProductPrice">{price_format(product,#listPrice#):h}</FONT><FONT class="ProductPriceTitle"> {product.priceMessage:h}</FONT><br><widget module="ProductAdviser" template="modules/ProductAdviser/PriceNotification/category_button.tpl" product="{product}" visible="{!getPriceNotificationSaved(product.product_id)}"><br></span>
	</td>
</tr>
<tr>
	<td FOREACH="row,product" align="center" width="{getPercents(config.ShowcaseOrganizer.so_columns)}%" valign="top">
