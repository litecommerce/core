...
<p>
<span class="SuccessMessage" IF="updated">NetworkMerchants parameters were successfully changed. Please make sure that the NetworkMerchants payment method is enabled on the <a href="admin.php?target=payment_methods"><u>Payment methods</u></a> page before you can start using it.</span>
...
<tr>
	...
	<td>
		<select name=params[testmode]>
			<option value="auth" selected="{IsSelected(pm.params.testmode,#auth#)}">Authorization</option>
			<option value="sale" selected="{IsSelected(pm.params.testmode,#sale#)}">Sale</option>
		</select>
	</td>
</tr>
