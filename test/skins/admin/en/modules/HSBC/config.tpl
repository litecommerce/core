<p>
<span class="SuccessMessage" IF="dialog.updated">HSBC parameters were successfully changed. Please make sure that the HSBC payment method is enabled on the <a href="admin.php?target=payment_methods">Payment methods</a> page before you can start using it.</span>
...
<tr>
	...
	<select name=params[param03]>
		...
		<option value=P selected="{IsSelected(dialog.pm.params.param03,#P#)}">live</option>
	</select>
	</td>
</tr>
...
<tr>
	...
	<select name=params[param05]>
		<option value="auth" selected="{IsSelected(dialog.pm.params.param05,#auth#)}">Auth</option>
		<option value="capture" selected="{IsSelected(dialog.pm.params.param05,#capture#)}">Capture</option>
	</select>
	</td>
</tr>
...
<tr>
	...
	<select name=params[param04]>
		...
		<option value="826" selected="{IsSelected(dialog.pm.params.param04,#826#)}">Pound Sterling</option>
		<option value="840" selected="{IsSelected(dialog.pm.params.param04,#840#)}">US Dollar</option>
	</select>
	</td>
</tr>
...
<tr>
	<td>Gateway URL:</td>
	<td><input type=text name=params[param09] size=48 value="{dialog.pm.params.param09}"></td>
</tr>
