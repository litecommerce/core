...
function PaymentMethod_2checkout_handleRequest($_this, $cart)
{
    ...
    if (isset($_POST['x_amount'])) {
        ...
        if ($total != $postTotal) {
			...
			die("<font color=red><b>Security check failed!</b></font> Please contact administrator <b>" . $_this->config->get("Company.site_administrator") . "</b> .");
        }
	}
...
function PaymentMethod_2checkout_v2_handleRequest($_this, $cart, $security_check=true)
{
	...
	if (!$security_check) {
		die("<font color=red><b>Security check failed!</b></font> Please contact administrator <b>" . $_this->config->get("Company.site_administrator") . "</b> .<hr>Click <a href=\"" . $_this->xlite->shopUrl("cart.php?target=checkout&mode=error&order_id=".$cart->get("order_id")) . "\"><u>here</u></a> to return into your cart.");
	}
