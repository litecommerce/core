...
* @version $Id$
*/
class XLite_Module_SnsIntegration_Controller_Admin_Module extends XLite_Controller_Admin_Module implements XLite_Base_IDecorator
{
	function init()
	{
		parent::init();
	...
	function checkSNSConnection()
	{
		...
		$actions[]= $action;
		$result = func_sns_request($this->config, func_get_sns_client_id(), $actions);

		if ($result) {
			switch ($this->xlite->get("SNSResponse")) {
				case "+OK":
					$this->set("SNSErrCode", 0);
				break;
				case "-Access denied":
					$this->set("SNSErrCode", 1);
				break;
				default:
					$result = false;
				break;
