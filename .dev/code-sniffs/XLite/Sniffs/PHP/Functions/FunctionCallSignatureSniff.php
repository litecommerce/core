<?php
/**
 * XLite_Sniffs_PHP_Functions_FunctionCallSignatureSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class XLite_Sniffs_PHP_Functions_FunctionCallSignatureSniff extends XLite_ReqCodesSniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
		...
		$var = $openBracket;
		do {
			$var = $phpcsFile->findNext(T_VARIABLE, $var + 1, $closeBracket);
			$anonFunc = $phpcsFile->findNext(T_FUNCTION, $var + 1, $closeBracket);
			if ($var && $anonFunc) {
				$var = $tokens[$anonFunc]['scope_closer'];
				continue;
			}
        ...
        if (($stackPtr + 1) !== $openBracket) {
            ...
            $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.7.3') . $error, $stackPtr);
        }
        ...
        if ($tokens[$next]['code'] === T_SEMICOLON) {
            if (in_array($tokens[($closeBracket + 1)]['code'], PHP_CodeSniffer_Tokens::$emptyTokens) === true) {
                ...
                $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.7.4') . $error, $closeBracket);
            }
        }
    ...
    public function processSingleLineCall(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        if ($tokens[($openBracket + 1)]['code'] === T_WHITESPACE) {
            ...
            $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.7.6') . $error, $stackPtr);
        }
        ...
        if ($tokens[($closer - 1)]['code'] === T_WHITESPACE) {
            ...
            if ($between !== $closer) {
                ...
                $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.7.6') . $error, $closer);
            }
        }
    ...
    public function processMultiLineCall(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        ...
        for ($i = ($openBracket + 1); $i < $closeBracket; $i++) {
            ...
            if ($tokens[$i]['line'] !== $lastLine) {
                ...
                if ($expectedIndent !== $foundIndent) {
					$rc = $i == $closeBracket - 1 ? 'REQ.PHP.2.7.9' : 'REQ.PHP.2.7.7';
        ...
        if ($tokens[($openBracket + 1)]['content'] !== $phpcsFile->eolChar) {
            ...
            $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.7.8') . $error, $stackPtr);
        }

        $prev = $phpcsFile->findPrevious(T_WHITESPACE, ($closeBracket - 1), null, true);
        if ($tokens[$prev]['line'] === $tokens[$closeBracket]['line']) {
            $error = 'Closing parenthesis of a multi-line function call must be on a line by itself';
            $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.7.9') . $error, $closeBracket);
        }
