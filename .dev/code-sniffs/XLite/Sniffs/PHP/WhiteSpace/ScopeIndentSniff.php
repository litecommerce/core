<?php
/**
 * XLite_Sniffs_Whitespace_ScopeIndentSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class XLite_Sniffs_PHP_WhiteSpace_ScopeIndentSniff extends XLite_ReqCodesSniff
{
   ...
   /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
		...
		if (
			...
			&& $tokens[$firstToken]['code'] == T_FUNCTION
			&& isset($tokens[$firstToken]['nested_parenthesis'])
			&& is_array($tokens[$firstToken]['nested_parenthesis'])
		) {
			$expectedIndent += count($tokens[$firstToken]['nested_parenthesis']) * $this->indent;
		}
        ...
        if ($tokens[$firstToken]['column'] !== $expectedIndent) {
            ...
            $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.4.1') . $error, $stackPtr);
        }
            ...
            if ($newline === true && $tokens[$i]['code'] !== T_WHITESPACE) {
                ...
                if ($column !== $indent) {
                    if ($this->exact === true || $column < $indent) {
                        ...
                        $error .= ($column - 1);
						$rq = $tokens[$tokens[$scopeOpener]['scope_condition']]['code'] === T_CLASS ? 'REQ.PHP.3.4.3' : 'REQ.PHP.2.4.1';
                        $phpcsFile->addError($this->getReqPrefix($rq) . $error, $firstToken);
                    }
                }
            }//end if
        }//end for
