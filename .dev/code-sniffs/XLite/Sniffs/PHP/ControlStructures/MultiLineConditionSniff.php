<?php
/**
 * XLite_Sniffs_PHP_ControlStructures_MultiLineConditionSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class XLite_Sniffs_PHP_ControlStructures_MultiLineConditionSniff extends XLite_ReqCodesSniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
            ...
            if ($tokens[$i]['line'] !== $lastLine) {
                if ($tokens[$i]['line'] === $tokens[$closeBracket]['line']) {
                    ...
                    if ($next !== $closeBracket) {
                        ...
                        $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.6.7') . $error, $i);
                ...
                if ($expectedIndent !== $foundIndent) {
                    ...
                    $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.6.6') . $error, $i);
                }

                if ($tokens[$i]['line'] !== $tokens[$closeBracket]['line'] && $tokens[$i]['line'] !== $tokens[$openBracket]['line'] + 1) {
                    $next = $phpcsFile->findNext(T_WHITESPACE, $i, null, true);
                    if (in_array($tokens[$next]['code'], PHP_CodeSniffer_Tokens::$booleanOperators) === false) {
                        ...
                        $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.6.5') . $error, $i);
                    }
                }
        ...
        if ($length !== 1) {
            ...
            $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.5.4') . $error, ($closeBracket + 1));
        }
        ...
        if ($next !== false && $tokens[$next]['code'] !== T_OPEN_CURLY_BRACKET) {
            ...
            $phpcsFile->addError($this->getReqPrefix('REQ.PHP.2.5.4') . $error, $next);
        }
