...
function change_color_GiftsShop(id, type, last, first, arrow)	{
...
function change_color_root_GiftsShop(id, type, selected)	{
...
function change_root_GiftsShop(id, type)	{
	tdid = "td_" + id; td2id = "td2_" + id;
	if(document.getElementById(tdid))
		change_color_root_GiftsShop(id, type);
	else	{
		last = cat_ids[id][0];
		first = cat_ids[id][1];
		arrow = cat_ids[id][2];
		change_color_GiftsShop(id, type, last, first, arrow);
	}
}
	...
	if(cat_id != 0)	{
		...
		for (var x = 0; x < parents[cat_id].length; x++)	{
			...
			if(current_id != cat_id)	{
				change_root_GiftsShop(current_id, type);
			}
		}
	}
}

//**************  end GiftsShop SCHEME  *************************
...
function CategoryBody_GiftsShop(id, href, caption, cat_id, first)
{
...
function SubCategoryBody_GiftsShop(id, href, caption, show_arrow, iconUrl, showIcon, cat_id, is_first, is_last, arrow)
{
	obj = document.getElementById(id);
    if ( !obj ) return;
