...
//*********************  SummerSports ********************
function change_color_SummerSports(id, type, prefix, selected)	{
...
function change_parents_color_SummerSports(cat_id, type)	{
	...
	for (var x = 0; x < parents[cat_id].length; x++)	{
		...
		if(current_id != cat_id)	{
			...
			change_color_SummerSports(current_id, type, prefix);
		}
	}
}
//**********************  End (SummerSports)  *******************
...
function CategoryBody_SummerSports(id, href, caption)
{
...
function SubCategoryBody_SummerSports(id, href, caption)
{
	obj = document.getElementById(id);
	if ( !obj ) return;
	line1_id = id + "_line1";
