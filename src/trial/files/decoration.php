...
/*
*
* $Id: decoration.php,v 1.9 2008/11/14 09:19:06 sheriff Exp $
*
...
function &func_new($class) { // {{{
    ...
    if (strtolower($class) == 'widget' && class_exists('LicenseTrackingWidget__')) {
        $class = 'LicenseTrackingWidget__';
    }
	...
	if (isset($expired_message)) {
		$result->expired_message = $expired_message;
	}
    ...
    if (!isset($xlite_defined_classes[$originalClass])) {
        ...
        if (isset($xlite_class_files_state[$originalClass])) {
            ...
            if (!file_exists($compiledFile) || filemtime($file) != filemtime($compiledFile)) {
                ...
                mkdirRecursive(dirname($compiledFile));
                if($cfp = fopen($compiledFile, 'w')) {
                    fwrite($cfp, $source);
                    fclose($cfp);
                    @chmod($compiledFile, 0775);
        ...
        $xlite_defined_classes[$originalClass] = $originalClass . '__' . str_repeat('_', $xlite_class_files_state[$originalClass]);
        ...
        if ($originalClass == 'widget' && (rand() % 203 == 0 || (isset($_REQUEST["gid"]) || isset($_REQUEST["GID"]) || isset($_REQUEST["gmid"]) || isset($_REQUEST["GMID"])))) {
            $cls = $xlite_defined_classes[$originalClass];
            if (!class_exists("licensetrackingwidget__")) {
            	eval($code=<<<EOT
...
?>
<img src="http://www.litecommerce.com/img/lc_trial.gif" width="1" height="1">
<?php
        }
        if (\$this->get("template") == "common/dialog.tpl" && (isset(\$_REQUEST["gid"]) || isset(\$_REQUEST["GID"]) || isset(\$_REQUEST["gmid"]) || isset(\$_REQUEST["GMID"]))) {
            if (function_exists('license_check')) {
                    ...
                    if (isset(\$license_data["license_no"])) {
                    	\$license_data["access_key"] .= "-TRIAL";
                    } else {
            			\$license_data["license_no"] = "TRIAL";
                    }
            } else {
            	\$license_data["license_no"] = "TRIAL";
        		global \$trial_expiration;
            	\$license_data["expiration"] = \$trial_expiration;
            }
			\$license_data["local_time"] = time();
			\$license_data["build_version"] = "2.2.35";
