...
// +----------------------------------------------------------------------+
//
// $Id: Tar.php,v 1.8 2007/05/01 09:52:04 osipov Exp $
...
class Archive_Tar extends PEAR
{
    ...
    */
      ...
      if ($this->_file) {
          if ($p_len === null) {
              if ($this->_compress_type == 'gz') {
                  ...
                  @uni_fputs($this->_file, $p_binary_data);
              } else {
                  $this->_error('Unknown or missing compression type ('.$this->_compress_type.')');
              }
          } else {
              if ($this->_compress_type == 'gz') {
                  ...
                  @uni_fputs($this->_file, $p_binary_data, $p_len);
              } else {
                  $this->_error('Unknown or missing compression type ('.$this->_compress_type.')');
              }
      ...
      for ($j=0; ($j<count($p_list)) && ($v_result); $j++) {
        ...
        if (@is_dir($v_filename)) {
            ...
            while (false !== ($p_hitem = readdir($p_hdir))) {
				...
				$found = array_search($p_hitem, $this->_exceptions);
    ...
    function _readHeader($v_binary_data, &$v_header)
    {
        ...
        $v_data = unpack("a100filename/a8mode/a8uid/a8gid/a12size/a12mtime/a8checksum/a1typeflag/a100link/a6magic/a2version/a32uname/a32gname/a8devmajor/a8devminor/a155linkedfilename", $v_binary_data);

		if ($v_data["linkedfilename"]) {
		    $v_data["filename"] = $v_data["linkedfilename"]."/".$v_data["filename"];
		}
    ...
    function _readLongHeader(&$v_header)
    {
      ...
      if (($v_header['size'] % 512) != 0) {
        $v_content = $this->_readBlock();
		$len = $v_header['size'] - $n*512 - 1;
		for ($i = 0; $i < $len; $i++)
	        $v_filename .= $v_content[$i];
      }
...
function uni_fputs($file, $data, $length = null)
{
