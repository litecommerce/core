...
CODE;
    ...
    {
        empty($substitutes) ?: $this->addReplacement($class, $this->substituteTemplate($template, $substitutes));
    }

    /**
     * Return the array of substitutes for the "translation" template
     * 
     * @param string class current multilang model class name
     ...
     * @since  3.0.0
     */
    protected function getTranslationSubstitutes($class, $name)
    {
        return !$this->getClassMetadata($class)->reflClass->hasProperty(
            'translations'
        ) ? array('____TRANSLATION_CLASS____' => $this->prepareClassName($name)) : array();
    }
     ...
     * @since  3.0.0
     */
    protected function getMethodSubstitutes($class, $entry, $method, $field)
    {
        return !$this->getClassMetadata($class)->reflClass->hasMethod(
            $method .= \Includes\Utils\Converter::convertToCamelCase($field)
        ) ? array($entry => $method) : array();
    }
    ...
    public function executeHookHandlerRun()
    {
        ...
        foreach ($this->getMultilangModelClasses() as $node) {
            ...
            if (!$translation) {
                ...
                $this->replace($main, 'translation', $this->getTranslationSubstitutes($main, $translation));
            }
            ...
            foreach ($this->getTranslationFields($translation) as $field) {
                ...
                foreach ($this->getAutogeneratedMethodsList() as $method => $entry) {
                    $this->replace($main, $method, $this->getMethodSubstitutes($main, $entry, $method, $field));
                }
            }
