...
* @package Dialog
* @access public
* @version $Id: memberships.php,v 1.1 2006/07/11 06:38:12 sheriff Exp $
*
*/

class Admin_Dialog_memberships extends Admin_Dialog // {{{
{
    ...
    function action_update() // {{{
    {
		if ($this->get("update_memberships")) {
			...
			foreach($memberships as $id => $membership_) {
				...
				$profiles[$id] = $profile->findAll("membership = '".$membership_->get("membership")."'");
			}	
		   	...
		   	foreach($memberships as $id => $membership_) {
				...
				if (isset($profiles[$id])) {
					foreach($profiles[$id] as $profile) {
						$profile->set("membership", $membership->get("membership"));
						$profile->update();	
					}
				}
		   }
