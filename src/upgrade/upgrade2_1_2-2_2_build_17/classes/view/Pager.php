...
* @package View
* @access public
* @version $Id: Pager.php,v 1.1 2006/07/11 06:38:30 sheriff Exp $
*/
    ...
    function &getPageData()
    {
        ...
        if (is_array($pageData)) {
        	for($i=0; $i<count($pageData); $i++) {
        		if (is_array($pageData[$i]) && isset($pageData[$i]["class"]) && isset($pageData[$i]["data"])) {
            		$object =& func_new($pageData[$i]["class"]);
                    $object->isPersistent = true;
                    $object->isRead = false;
                    $object->properties = $pageData[$i]["data"];
                    $pageData[$i] = $object;
        		}
        	}
        }
