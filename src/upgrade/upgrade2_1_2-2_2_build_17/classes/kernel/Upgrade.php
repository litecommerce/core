...
* @package Kernel
* @access public
* @version $Id: Upgrade.php,v 1.2 2006/07/14 12:39:05 sheriff Exp $
*/
class Upgrade extends Base
{
    ...
    */
    function doUpgrade() 
    {
        ...
        if ($from_ver != $this->config->Version->version) {
        	if ($this->config->Version->version != "2.1.2") {
            	die("<font color=red>Can't apply this patch to LiteCommerce version ".$this->config->Version->version.": patch version doesn't match or patch has been applied already</font>");
            }
        }
...
<?php
	...
	function success()
	{
		...
		if ($this->get("from_ver") == $this->config->Version->version) {
			$this->set("date", time());
			$this->create();
    ...
    function copyFile($from, $to)
    {
        ...
        $this->status($result = @copy($from, $to));
        @umask(0);
        @chmod ($to, 0777);
		if (!$result) {
			$this->failed = true;
