...
define('MODULE_COMMERCICAL_OTHER', 2);
define('MODULE_COMMERCICAL_SHIPPING', 4);
define('MODULE_COMMERCICAL_PAYMENT', 8);
...
* @package Kernel
* @access public
* @version $Id: Module.php,v 1.1 2006/07/11 06:38:24 sheriff Exp $
*/
    ...
    */
	...
	function checkModuleType() // {{{
	{
		if ($this->xlite->_shippingMethodRegistered == 1) {
			$moduleType |= MODULE_COMMERCICAL_SHIPPING;
		}

		if ($this->xlite->_paymentMethodRegistered == 1) {
			$moduleType |= MODULE_COMMERCICAL_PAYMENT;
		}
		...
		if ($this->isFree) {
			...
			if (!$this->xlite->mm->get("safeMode")) {
    			if (($moduleType && (MODULE_COMMERCICAL_SHIPPING | MODULE_COMMERCICAL_PAYMENT)) == 0) {
    				$moduleType |= MODULE_COMMERCICAL_OTHER;
    			}
    		}
		}
