...
* @package kernel
* @access public
* @version $Id: Membership.php,v 1.1 2006/07/11 06:38:24 sheriff Exp $
*/
class Membership extends Base // {{{
{
	...
	function create() // {{{
	{
		...
		$memberships[$max] = $this->get("properties");
        $config =& func_new("Config");
	...
	function update() // {{{
	{
	    $config =& func_new("Config");
        $memberships = $this->get("memberships");
		$memberships[$this->get("membership_id")] = $this->get("properties");
		$config->createOption("Memberships","membershipsCollection", serialize($memberships));
    } // }}}
