...
* @package kernel
* @access public
* @version $Id: Product.php,v 1.1 2006/07/11 06:38:24 sheriff Exp $
*/
class Product extends Base
{
    ...
    */
    ...
    function _importImage($product, $type, $name, $save_images) // {{{
    {
		...
		if ($config->find("name='images_directory'")) {
			$images_directory = $config->get("value");
		}
        $image_path = empty($images_directory) ? $name : ("$images_directory". ((substr($images_directory,strlen($images_directory)-1,1)!=DIRECTORY_SEPARATOR) ? DIRECTORY_SEPARATOR : "" )."$name");
