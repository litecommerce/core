...
* @package Kernel
* @access public
* @version $Id: ModulesManager.php,v 1.3 2006/07/14 12:39:05 sheriff Exp $
*/
    ...
    */
        ...
        while ($module->next($result)) {
            ...
            if ($mod->is("enabled")) {
            	...
            	if ($mod->is("enabled") && !$this->get("safeMode")) {
	            	$mod =& func_get_instance("Module_$name", $name); // define and init
                	$mod->init();
                }
				$mod->set("type", $moduleType);
				$mod->checkModuleType();
				if (($mod->get("type") & MODULE_3RD_PARTY) != 0) {
					$mod->set("type", $mod->get("type") & (MODULE_3RD_PARTY+MODULE_COMMERCICAL_SHIPPING+MODULE_COMMERCICAL_PAYMENT));
				}
    ...
    function getPredefinedModuleType($moduleName)
    {
    	switch($moduleName) {
			...
			case "ChronoPay":
			case "BeanStream":
    		return (MODULE_COMMERCICAL_PAYMENT);
    		case "GiftCertificates":
    		case "Promotion":
    		return (MODULE_COMMERCICAL_OTHER | MODULE_COMMERCICAL_PAYMENT);
    		case "AustraliaPost":
    		case "CanadaPost":
    		case "Intershipper":
    		case "UPS":
    		case "USPS":
    		return (MODULE_COMMERCICAL_SHIPPING);
    		case "AOM":
    		...
    		case "WishList":
    		return MODULE_COMMERCICAL_OTHER;
    		case "AdvancedSearch":
...
EOT;
		...
		$dest_file = "var/tmp/" . $_FILES["module_file"]["name"];
		move_uploaded_file($_FILES["module_file"]["tmp_name"], $dest_file);
