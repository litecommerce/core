...
* @package Kernel
* @access public
* @version $Id: Image.php,v 1.1 2006/07/11 06:38:24 sheriff Exp $
**/
class Image extends Base
{
	...
	*/
    ...
    function copyTo($id)
    {
        $newImg = func_new("Image",$this->imageClass, $this->get($this->autoIncrement));
        if (!$this->isRead) {
            $this->read();
        }
	...
	**/
	...
	function upload($image_field, $filesystem = false, $force = false)
	{
	    if (isset($_FILES[$image_field]) && is_uploaded_file($_FILES[$image_field]["tmp_name"])) {
        	$dest_file = "var/tmp/" . $_FILES[$image_field]["name"];
        	move_uploaded_file($_FILES[$image_field]["tmp_name"], $dest_file);
	...
	function import($image_file, $source = 'D', $filename = '', $force = false)
	{
		...
		if ($source == "D") {
			...
			if (is_uploaded_file($image_file)) {
				move_uploaded_file($image_file, $filepath);
			} else {
				copy($image_file, $filepath);
			}
			@chmod($filepath, 0644);
    ...
    function getImageType($image_file)
    {
		...
		$r = @getimagesize($image_file) or die("<font color=red>Invalid image file or file not found: $image_file</font>");
	...
	function moveToFilesystem($from = false)
	{
		...
		foreach ($imagesArray as $row) {
			...
			if ($from) {
				...
				if ($imagesHash[$fn] == 0) {
                    if (!@unlink($fn)) {
                        print "WARNING: unable to delete file $fn: permission denied<br>";
                    }
                }
			} else {
                ...
                if (is_writable($image->getFilePath("."))) {
					if (($fd = fopen($filePath, "wb"))) {
						...
						print "Error: directory '".$image->getFilePath("."). "' is not writable!<br>\n";
						func_flush();
					}
				} else {
					print "Error: file '$filePath' is not writable!<br>\n";
					func_flush();
				}
			}
		}
...
class ImageClass extends Object
{
    /**
