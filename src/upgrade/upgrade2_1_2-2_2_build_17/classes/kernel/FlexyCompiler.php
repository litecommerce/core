...
* @package Kernel
* @access public
* @version $Id: FlexyCompiler.php,v 1.1 2006/07/11 06:38:24 sheriff Exp $
*/
class FlexyCompiler extends Object
{
	...
	function html()
	{
		while ($this->phptag() || $this->tag() || $this->comment() || $this->flexy() || $this->anyChar()) {
		}
		return true;
	}
    ...
    function widgetDisplayCode($attrs)
    {
        $result = 'if($t->isDisplayRequired(array(';
        foreach ($attrs as $a => $v) {
            $result .= '"'.$a.'"=>' . $this->flexyAttribute($v) . ',';
        }
        $result .= "))){";
        ...
        foreach ($attrs as $name=>$value) {
            ...
            $result .= '$t->set(\'' . $wName . '.' . $name . '\',' . $this->flexyAttribute($value) . ');'; 
        }
