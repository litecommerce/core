...
* @package kernel
* @access public
* @version $Id: online.php,v 1.1 2006/07/11 06:38:27 sheriff Exp $
*/
class Shipping_online extends Shipping
{
    var $optionsTable;
    ...
    function unserializeCacheRates($rates)
    {
        ...
        $total = (double) $order->get("subtotal");
        ...
        if (!empty($rates)) {
            foreach (explode(',', $rates) as $rate) {
                ...
                $rateObject = func_new("ShippingRate",$shipping_id);
                ...
                $rateObject->shipping = func_new("Shipping",$shipping_id);
                if ($rateObject->shipping->is("exists") && $rateObject->shipping->is("enabled")) {
                    // haven't we remove this shipping?
                    $rateObject->rate = (double)$rate_value + $rateObject->get("flat") + $rateObject->get("per_item") * $items + $rateObject->get("percent")*$total/100 + $rateObject->get("per_lbs")*$weight;
    ...
    function _checkCache($table, $fields)
    {
        $cacheTable = $this->db->getTableByAlias($table);
        // garbage collection
        $this->db->query("DELETE FROM $cacheTable WHERE date<".(time()-SHIPPING_CACHE_EXPIRATION)); 
