...
Select a country or a state from a list, specify the zone where the country or state should be listed and click on the 'Apply' button. To select more than one country/state, hold down the CTRL key while making a selection. One country or state can only be listed in one zone.
</span>

<hr>

<table border=0 cellpadding=5>
<tbody FOREACH="xlite.factory.ShippingZone.findAll(),zone">
...
<option FOREACH="xlite.factory.ShippingZone.findCountryZones(),z" value="{z.shipping_zone}">{z.name}</option>
</select><br>
...
<option value="new">New zone</option>
<option FOREACH="xlite.factory.ShippingZone.findStateZones(),z" value="{z.shipping_zone}">{z.name}</option>
</select><br>
