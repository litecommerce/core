...
<p align="justify">
SHIPPING = raw on-line_rate + flat + ITEMS*per_item + WEIGHT*per_lbs + SUBTOTAL*(% of subtotal)/100;
</p>										  
<p align="justify">
...
<table border="0">
	...
	<tr>
		<td>
			...
			<option FOREACH="shippings,shipping" value="{shipping.shipping_id}" selected="{isSelected(shipping,#shipping_id#,shipping_id_range)}">{shipping.name}</option>
			</select>
		</td>
		<td>
			<select name="shipping_zone_range" onChange="document.charges_methods.submit();">
			<option value="">All shipping zones</option>
			<option FOREACH="xlite.factory.ShippingZone.findAll(),zone" value="{zone.shipping_zone}" selected="{isSelected(zone,#shipping_zone#,shipping_zone_range)}">{zone.name}</option>
			</select>
		</td>
	</tr>
</table>
