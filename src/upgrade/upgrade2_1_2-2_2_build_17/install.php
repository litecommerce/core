...
*
* $Id: install.php,v 1.1 2006/07/11 06:38:33 sheriff Exp $
*
...
$directories_to_create = array("var", "$templates_directory", "catalog","images");
...
if ($params["force_current"] == get_step('check_cfg')) {
...
?>
.background {
	BACKGROUND-COLOR: #FFFFFF; BACKGROUND-IMAGE: URL('http://www.litecommerce.com/img/logo_lite.gif');
}
<?php
}else {
?>
...
?>
  ...
  <INPUT type=button value="&lt; Back"<?php echo ($prev > 0 ? "" : " disabled") ?> onClick="javascript:return step_back()">
  <INPUT type=button value="Next &gt;"<?php echo ($error ? " disabled" : "") ?> onClick="javascript:if (step_next()) { ifrm.submit(); return true; } else { return false; }">
 </TD>
</TR>
<?php
}
...
<DIV ALIGN=right>
  <FONT size=1>
  Copyright &copy; 2003 - 2006 <A href="http://www.creativedevelopment.biz">Creative Development</A>
</DIV>
</BODY>
</HTML>
<?php
...
function check_ftp() // {{{
{
	...
	ob_end_clean(); 
	return preg_match("/.+(--enable-ftp).+/",$info);
} // }}}	
...
{	
	...
	{
		warning_error("FTP connection not tested because some part of related information is missing.");		
		return false;
	}	
...
<TEXTAREA name="copyright" cols="80" rows="22" style="font-family: courier; FONT-SIZE: 10px">
<?php readfile("./COPYRIGHT"); ?>
</TEXTAREA>
...
<TABLE border=0>
<TR><TD><input type=radio name="params[force_current]" value="<?php print get_step("check_cfg") ?>"><TD><b>Perform new installation</b></TD></TR>
<TD><input type=radio name="params[force_current]" value="<?php print get_step("cfg_install_dirs") ?>" checked></TD><TD><b>Re-install skin files</b></TD></TR>
<TD><input type=radio name="params[force_current]" value="<?php print get_step("cfg_create_admin") ?>"></TD><TD><b>Configure primary administrator account</b></TD></TR>
...
<INPUT type=checkbox name="agree" onClick="this.blur()"> I accept the License Agreement
...
function module_check_cfg(&$params) { 
	...
	if (isset($params["xlite_http_host"])) unset($params["xlite_http_host"]);
	if (isset($params["xlite_https_host"])) unset($params["xlite_https_host"]);
	if (isset($params["xlite_web_dir"])) unset($params["xlite_web_dir"]);
	if (isset($params["mysqlhost"])) unset($params["mysqlhost"]);
	if (isset($params["mysqlbase"])) unset($params["mysqlbase"]);
	if (isset($params["mysqluser"])) unset($params["mysqluser"]);
	if (isset($params["mysqlpass"])) unset($params["mysqlpass"]);
...
<TABLE width="100%" border=0 cellspacing=0 cellpadding=4>
 ...
 <TR class="Clr<?php echo $clrNumber; $clrNumber = ($clrNumber == 2) ? 1 : 2; ?>">
  <TD colspan=3 align=left><B>Inspecting server configuration</B></TD>
 </TR>
...
// PHP Version must be at least $min_ver
	...
	$ver = phpversion();
	$min_ver < $ver && $ver < $max_ver ? $status = 1: $status = 0;
	$unsupported_version = $ck_res = $status;
    foreach($forbidden_versions as $fpv) {
    	if ($ver >= $fpv["min"] && $ver <= $fpv["max"]) {
...
?>
 <TR class="Clr<?php echo $clrNumber; $clrNumber = ($clrNumber == 2) ? 1 : 2; ?>">
  ...
  <TD width="1%">-</TD>
  <TD width="1%" align=center><?php if (!$res) echo status($res); else { ?>
...
?>
    ...
    <font style="font-size: 14px; color: red">Dependency failed: Unsupported version of PHP (<?php echo phpversion(); ?>)</font>
    <br><br>
	Currently versions <b>4.1.0</b> - <b>4.4.X</b> and <b>5.1.X</b> are supported. <BR><BR>
...
<?php
	} elseif (!$safe_mode_status) {
        $info = get_info();
?>

    <font style="font-size: 14px; color: red">Dependency failed: Safe Mode is ON</font>
    <br><br>
    ...
    <br><br>
    Please contact your Hosting Support Service to adjust this parameter.
    <br><br>
<?php
	} elseif (!$allow_url_fopen_status) {
        $info = get_info();
?>

    <font style="font-size: 14px; color: red">Dependency failed: allow_url_fopen</font>
    <br><br>
	For LiteCommerce application to work correctly, allow_url_fopen variable in php.ini must be turned ON.
	...
	<br><br>
<?php
    } elseif (!$ioncube_status) {
?>
    <font style="font-size: 14px; color: red">Dependency failed: ionCube loader</font>
    <br><br>
<?php
        $info = get_info();
...
?>
    ...
    <a href="http://www.ioncube.com/loader_download.php" target="_blank"><u>ionCube loader</u></a> to operate.<br>Failed dependency means that either your Web server is multithreaded<br>or does not support dynamic library loading,<br>or ionCube loader is not installed.
    ...
    <p><b>2. If you do not have access to php.ini file</b>
    <br><br>
	Try to download the latest version of ionCube loader for your Web server<br><br><font style="background-color: #E3EAEF;"><?php print $loader_dll ?></font><br><br>from <a href="http://www.ioncube.com/loader_download.php" target="_blank"><u>ionCube site</u></a> and upload it into <b>ioncube/</b> directory<br>in your LiteCommerce installation folder via FTP<br>or other means that you use to manage your LiteCommerce software.
    <br><br>

    <p><b>3. If you do not feel confident about doing item 2 by yourself</b>
    <br><br>
	Please contact the support services of your hosting provider<br>to have the ionCube loader installed for you.
    <br><br>
...
if (substr(PHP_OS, 0, 3) != 'WIN') {
    if (!is_writable(".")) $perms[] = "&gt; chmod 777 .";
    if (!is_writable($config_file)) $perms[] = "&gt; chmod 666 $config_file";
    if (!is_writable("classes/modules")) $perms[] = "&gt; chmod 777 classes/modules";
/*	if (!is_writable("schemas")) $perms[] = "&gt; chmod 777 schemas";
	if (!is_writable("skins_original")) $perms[] = "&gt; chmod 777 skins_original";*/
...
function module_cfg_install_db(&$params) {
	...
	if (check_ftp()&&isset($params['ftp_enabled'])) {
		...
		if ($ftp_connection) {
			...
			ftp_site($ftp_connection,"CHMOD 0777 .");
			ftp_site($ftp_connection,"CHMOD 0777 classes/modules");
			ftp_site($ftp_connection,"CHMOD 0666 etc/config.php");
...
<TABLE width="100%" border=0 cellpadding=4>
 ...
 <TR class="Clr<?php echo $clrNumber; $clrNumber = ($clrNumber == 2) ? 1 : 2; ?>">
  ...
  <TD>
   <SELECT name="params[states]">
    <OPTION value=US>USA states</OPTION>
 ...
 <TR class="Clr<?php echo $clrNumber; $clrNumber = ($clrNumber == 2) ? 1 : 2; ?>">
  ...
  <TD>
  	<INPUT type=checkbox name="params[images_to_fs]" value="Y" onClick="this.blur()">
  </TD>
 </TR>
 ...
 <TR id="preview_container" style="display: none;">
  <TD colspan=2>
	<TABLE width="640" border=0 cellpadding=0 cellspacing=0 align=center>
  		<TR>
  		<TD width=640><DIV style="overflow: auto; height: 350; width: 660"><IMG id="preview_img" border=1 src="http://lite438.crtdev.local/~sheriff/lc_2_2/schemas/templates/2-columns_modern/preview.gif"></DIV></TD>
  		</TR>
	</TABLE>
  </TD>
 </TR>
...
?>
previewImages["<?php echo $key; ?>"] = "<?php echo "http://" . $params["http_host"] . $params["web_dir"] . "/" . $value["preview"]; ?>";
<?php
		}
	}
?>
...
function module_install_dirs(&$params) {
	global $directories_to_create, $files_to_create, $templates_repository, $schemas_repository, $error, $default_skin, $default_locale, $templates_directory, $config_file;
	$clrNumber = 2;
...
function module_create_admin(&$params) {
    ...
    if (!$mylink) {
		...
		@mysql_query("SET sql_mode='MYSQL40'");
        // check for profile
        $sql = "SELECT COUNT(*) FROM xlite_profiles WHERE login='$params[login]'";
        $query = "";
        ...
        if ($data[0]) {  // account already exists
            // update profile
            $query = "UPDATE xlite_profiles SET password='$password', status='E' WHERE login='$login'";
...
function module_install_done(&$params) {
    ...
    $authcode = get_authcode();
    // if authcode IS NULL, then this is probably the first install, skip
    if (isset($authcode) && isset($params["login"]) && isset($params["password"]) && isset($params["confirm_password"]) && strlen($params["login"].$params["password"].$params["confirm_password"]) > 0) {
        // create/update admin account from the previous step
        module_create_admin($params);
    }
	...
	if (!file_exists("install.php") && file_exists($install_name)) {
		$message =<<<EOF
Dear shop administrator!
	  
    To ensure the security of your LiteCommerce installation, the file "install.php" has been renamed to "$install_name". 
    Now, if you choose to change your store's skin set, add a new admin account or re-install LiteCommerce, you should rename the file "$install_name" back to "install.php" by using the following console command: 
    > mv $install_name install.php
...
EOF;
    	...
    	@mail($params["login"], "LiteCommerce installation security notice", $message);
	}
...
The following URL can be used to update primary administrator profile,
change layout skins or completely reinstall LiteCommerce:
...
Your auth code for running install.php in the future is: <B><?php print get_authcode();  ?>&nbsp;&nbsp;&nbsp;- PLEASE WRITE THIS CODE DOWN UNLESS YOU ARE GOING TO REMOVE install.php</B>
<P>
'install.php' script can be used to update primary administrator profile,
change layout skins or completely reinstall LiteCommerce. If you do not need this functionality, you can delete 'install.php' installation script.
<?php
...
<BR>
<BR>
LiteCommerce software has been successfully installed and is now available at the following URLs:<BR>
<OL>
