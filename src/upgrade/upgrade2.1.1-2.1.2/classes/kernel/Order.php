...
* @package kernel
* @access public
* @version $Id: Order.php,v 1.1 2004/11/22 09:19:48 sheriff Exp $
* @see Cart
*/
class Order extends Base
{
    ...
    */
    function calcTotal() // {{{
    {
        ...
        $this->calcTax();
$total = $this->get("subtotal") + $this->get("tax") + $this->get("shippingCost");
$this->set("total", $this->formatCurrency($total));
    ...
    function addItem($item) // {{{
    {
        ...
        for ($i=0; $i<count($items); $i++) {
            if ($items[$i]->get("key") == $key) {
                // add quantity
                $items[$i]->set("amount", 
                        $items[$i]->get("amount") + $item->get("amount"));
                $items[$i]->update();
                return;
            }
        }
    ...
    */
    ...
    function search($profile, $id1, $id2, $status, $startDate, $endDate) // {{{
    {
        $where = array();
        if (isset($profile)) {
    ...
    */
    function succeed()
    {
        ...
        if ($this->config->get("Email.enable_init_order_notif") || $this->config->get("Email.enable_init_order_notif_customer"))
        {    
