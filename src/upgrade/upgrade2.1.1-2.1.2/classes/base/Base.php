...
* @package Base
* @access public
* @version $Id: Base.php,v 1.1 2004/11/22 09:19:48 sheriff Exp $
*/
class Base extends Object
{
    ...
    */
    ...
    /**
    * Shows whether the object data has been read from DB or not
    * @var boolean $isRead
    */
    var $isRead = false;
    ...
    */
    ...
    function iterate($where = null, $orderby = null, $groupby = null, $limit = null) // {{{
    {
        ...
        $result = $this->db->getAll($this->sql);
        if (!is_array($result)) {
            die ($this->sql.": ".$result->getMessage());
        }
		return $result;
	}
    ...
    */
    function delete() // {{{
    {
        ...
        // die otherwise
        func_die("Unable to delete unspecified row from " . $this->alias);
    } // }}}
    ...
    */
    ...
    function import(&$options) // {{{
    {
        ...
        is_array($options) or die("Invalid import options");

        $file = $options["file"];
        $handle = fopen($file, 'r') or die("Failed to open import file $file");
        ...
        while ($line = fgets($handle)) {
            ...
            if (is_null($columns)) {
                die("CVS syntax error in line ".$this->lineNo.": $error");
            }
            $properties = array();
            for ($i = 0; $i < count($layout); $i++) {
                if ($layout[$i] != "NULL") {
                    array_key_exists($i, $columns) or die("Ivalid CSV file: column count does not match");
                    $properties[$layout[$i]] = $columns[$i];
                }    
            }   
    ...
    {
        die("Base::_import() method should be overridden");
    } // }}}
    
    function _export() // {{{
    {
        die("Base::_export() method should be overridden");
    } // }}}
    ...
    function export($layout, $delimiter, $where = null, $orderby = null, $groupby = null) // {{{
    {
        ...
        do {
            ...
            foreach ($items as $item) {
                print func_construct_csv($item->_export($layout, $delimiter), $delimiter, '"');
                print "\n";
            }
            $processed += $limit;
            $items = array();
