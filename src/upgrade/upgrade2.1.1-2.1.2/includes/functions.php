...
function mkdirRecursive($dir, $mode = 0777)  // {{{
{
	...
	while ($newdir = array_shift($dirstack)) {
        ...
        umask(0000);
		@mkdir($newdir, $mode) or die("Can't create directory $newdir: permission denied");
	}
	return $ret;
} // }}}
...
function func_parse_url($url) // {{{
{
    ...
    return array_merge($parts_default, parse_url($url));
} // }}}
    ...
    function func_die($message) // {{{
    {
        if (function_exists("debug_backtrace")) {
            ...
            foreach (debug_backtrace() as $trace) {
                ...
                print $trace["class"]."::".$trace["function"] . "(". basename($trace["file"]).":".$trace["line"].") <b>args:</b> ".join($trace["args"], ',')."\n";
            }
        }
        die ($message);
    } // }}}
    ...
    for ($i=0; $i<=strlen($line); $i++) {
        ...
        if ($state == "outside") {
            ...
            if ($char == $q) {
                ...
                $error = "Unexpected ent of line; $q expected";
                return null;
            } else {
                $field .= $char;
            }
            ...
            if ($char == $q) { // double-quote
                ...
                $error = "Unexpected character $char outside quotes: $q expected";
                return null;
            }
        } elseif ($state == "field") {
