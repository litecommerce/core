...
* @package Dialog
* @access public
* @version $Id: taxes.php,v 1.4 2007/06/18 11:23:32 osipov Exp $
*
*/
class Admin_Dialog_taxes extends Admin_Dialog
{
    ...
    function action_update_rates()
    {
        ...
        if (isset($_POST["varvalue"])) {
            ...
            foreach ($_POST["varvalue"] as $ind => $value) {
                $value = trim($value);
                // check expression {{{
                if ($value{0} == '=' && !$tax->checkExpressionSintax($value, $this->invalidExpressions[$ind])) {
                    $this->invalidExpressions[$ind] = '"<b>'.join('</b>", "<b>', $this->invalidExpressions[$ind]).'</b>"';
                    continue;
                } else {
                    unset($this->invalidExpressions[$ind]);
                }
                // }}}
    ...
    function _readTaxForm()
    {
        ...
        if ($taxName !== '' && $taxValue !== '') {
            if (is_numeric($taxValue)) {
                ...
                $invalids = array();
                $tax =& func_new('TaxRates');
                if ($tax->checkExpressionSintax($taxValue, $invalids)) {
                    $action = "$taxName:=$taxValue";
                } else {
                    $this->set('inv_exp_error', 1);
    ...
    {
        return isset($this->invalidExpressions[$ind]);
    }
