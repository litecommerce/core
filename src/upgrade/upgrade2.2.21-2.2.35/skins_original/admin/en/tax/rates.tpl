...
<td colspan="{getColspan(ind)}">{getCondition(tax)}</td><td>{getCondVarName(tax)}</td>
<td><input type="text" name="varvalue[{ind}]" value="{getCurrentCondVarValue(ind,tax)}" size="15">
</td>
{end:}
{if:isAction(tax)}
<td colspan="{getColspan(ind)}">&nbsp;</td><td>{getVarName(tax)}</td><td><input type="text" name="varvalue[{ind}]" value="{getCurrentVarValue(ind,tax)}" size="15"></td>
{end:}
...
<tr IF="isInvalidExp(ind)">
    <td colspan="4"><table border="0" style="margin-left: 25px;">
        <tr><td><img src="skins/admin/images/code.gif" alt="[!]"></td><td>&nbsp;The {getExpInvalidVar(ind):h} variables are not defined, the tax rate cannot be calculated.</td></tr>
    </table></td>
</tr>
</tbody>
</table>
