<a href="admin.php?target=template_editor">Template Editor</a><br>
<a href="admin.php?target=css_edit">CSS Editor</a><br>
<a href="admin.php?target=image_edit">Image Editor</a><br>
<widget module="FancyCategories" template="modules/FancyCategories/look_feel/menu.tpl">
<widget module="LayoutOrganizer" template="modules/LayoutOrganizer/look_feel/menu.tpl">
<hr>
<a href="admin.php?target=wysiwyg" class="SidebarItem">Design import/export</a><br>
<a href="admin.php?target=change_skin" class="SidebarItem">Change current skin</a><br>
