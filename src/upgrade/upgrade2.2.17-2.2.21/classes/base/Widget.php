...
* @package Base
* @access public
* @version $Id: Widget.php,v 1.2 2006/08/23 10:13:00 sheriff Exp $
*/
class Widget extends Object
{
    ...
    */
    ...
    function includeCompiledFile($includeFileProp)
    {
        ...
        if (!file_exists($includeFile) || filemtime($includeFile) != filemtime($templateFile)) {
            ...
            $fc->parse();
            mkdirRecursive(dirname($displayFile), 0755);
            if($cfp = fopen($displayFile, 'wb')) {
                fwrite($cfp,$fc->get("phpcode"));
                fclose($cfp);
                @chmod($displayFile, 0644);
                @touch($displayFile, filemtime($templateFile));
            }
            mkdirRecursive(dirname($initFile), 0755);
    ...
    */
    ...
    {
        $price = sprintf("%.02f", doubleval($value));
        return $price;
    }
