...
* @package Kernel
* @access public
* @version $Id: FileNode.php,v 1.2 2006/08/23 10:13:00 sheriff Exp $
*/
...
class FileNode extends Object
{
    ...
    function rename()
    {
		...
		is_dir($this->newPath) ? @chmod($this->newPath, 0755) : @chmod($this->newPath, 0666);
    }
    ...
    function checkAccess($file)
    {
        ...
        foreach (explode('/', $file) as $element) {
            if ($element == '..') $i--;
            if ($element != 'skins' && $element != 'skins_original'&& $element != 'tests' && $i == 0) {
                $this->accessDenied();
            }
