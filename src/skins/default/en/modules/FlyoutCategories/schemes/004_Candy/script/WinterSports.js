...
//*********************  WinterSports ********************
function change_color_WinterSports(id, type, prefix, selected)	{
...
function change_parents_color_WinterSports(cat_id, type)	{
	...
	for (var x = 0; x < parents[cat_id].length; x++)	{
		...
		if(current_id != cat_id)	{
			...
			change_color_WinterSports(current_id, type, prefix);
		}
	}
}
//**********************  End (WinterSports)  *******************
...
function CategoryBody_WinterSports(id, href, caption)
{
...
function SubCategoryBody_WinterSports(id, href, caption)
{
	obj = document.getElementById(id);
	if ( !obj ) return;
	line1_id = id + "_line1";
