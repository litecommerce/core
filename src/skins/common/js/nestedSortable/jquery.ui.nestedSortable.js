...
(function(jQuery) {
	...
	jQuery.widget("ui.nestedSortable", jQuery.extend({}, jQuery.ui.sortable.prototype, {
		...
		_mouseDrag: function(event) {
			...
			var parentItem = (this.placeholder[0].parentNode.parentNode
			    ...
			    previousItem = this.placeholder[0].previousSibling ? jQuery(this.placeholder[0].previousSibling) : null;
		...
		toHierarchy: function(o) {
			...
			jQuery(this.element).children('li').each(function () {
			...
			function _recursiveItems(li) {
				var id = (jQuery(li).attr(o.attribute || 'id') || '').match(o.expression || (/(.+)[-=_](.+)/));
				if (id) {
					var item = {"id" : id[2]};
					if (jQuery(li).children(o.listType).children('li').length > 0) {
						item.children = [];
						jQuery(li).children(o.listType).children('li').each(function() {
							var level = _recursiveItems(jQuery(this));
		...
		toArray: function(o) {
			...
			ret.push({
				...
				"depth": sDepth,
				"left": '1',
				"right": (jQuery('li', this.element).length + 1) * 2
			});
			...
			function _recursiveArray(item, depth, left) {
				...
				if (jQuery(item).children(o.listType).children('li').length > 0) {
					depth ++;
					jQuery(item).children(o.listType).children('li').each(function () {
		...
		_isAllowed: function(parentItem, levels) {
			...
			if (
              ...
              || (
                ...
                && (
                  o.levelClass
                  && jQuery(this.currentItem[0]).hasClass(o.levelClass + '-' + levels)
                )
              )
            ) {
	...
	jQuery.ui.nestedSortable.prototype.options = jQuery.extend({}, jQuery.ui.sortable.prototype.options, jQuery.ui.nestedSortable.prototype.options);
})(jQuery);