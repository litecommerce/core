  ...
  <form action="admin.php" method="post" name="edit_label_form">
    ...
    <ul>
      ...
      <li FOREACH="getAddedLanguages(),l">
        <label for="edit_label_{l.code}" class="language" style="background-image: url({l.flagURL});">{l.name}<span IF="l.code=defaultLanguage.code"> ({t(#required#)})</span></label>
        <textarea id="edit_label_{l.code}" name="label[{l.code}]"{if:l.code=defaultLanguage.code} class="field-required"{end:}>{getTranslation(l.code)}</textarea>
      </li>
