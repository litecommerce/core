...
<div class="payment-settings {paymentMethod.getServiceName()}">
  ...
  <div IF="!{paymentMethod.getSetting(#hide_instruction#)}" class="footer">
    ...
    <div>To test PayPal Payments Advanced, you will need to set up two PayPal Sandbox accounts (one to simulate you, as the merchant, and one to simulate a buyer) and a test Payflow Link account. To set up these accounts, follow these instructions:</div>
    ...
    <div>
      <ul>
        ...
        <li>3. Set <b>Account Type</b> to <b>Website Payments Pro</b>.</li>
        ...
        <li>7. Create a test Payflow Link account by going to <a href="https://registration.paypal.com/welcomePage.do?producttype=C1&country=US&mode=try">the link</a> and following the registration process. When you reach the "Next Steps" page, your account is created.
        <li>8. Send an email to payflow-support@paypal.com with the subject line of <b>Link Sandbox Test Account to Gateway</b>. Explain that you are testing PayPal Payments Advanced, and that you would like to have your Sandbox account linked to your Payflow Link account for this purpose. Provide then with the Merchant Login name (which you chose during step 7), and the email address of the Sandbox merchant account (from step 5). PayPal will notify you by email when setup is complete.</li> 
      </ul>
    </div>
