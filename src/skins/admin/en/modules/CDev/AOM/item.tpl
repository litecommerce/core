...
{end:}
		...
		<table>	
			<tr>
				<td colspan="2">{if:clone}<widget module="CDev\ProductOptions" item="{item}" template="modules/CDev/AOM/product_options.tpl" IF="{item.hasOptions()}">{else:}<widget module="CDev\ProductOptions" item="{item}" template="modules/AOM/selected_options.tpl" IF="{item.hasOptions()}">{end:}</td>
			</tr>
			<tr>
		    	<td colspan="2">&nbsp;</td>
			</tr>
