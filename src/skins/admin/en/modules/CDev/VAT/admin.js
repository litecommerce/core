  ...
  function() {
    ...
    jQuery('.edit-vat-tax table.form button.switch-state')
      ...
      function() {
        ...
        core.post(
          URLHandler.buildURL({target: 'taxes', action: 'VATTaxSwitch', page: page}),
    ...
    {
      if (3 < jQuery('.edit-vat-tax table.data tr').length) {
        jQuery('.edit-vat-tax table.data').removeClass('empty-data');

      } else {
        jQuery('.edit-vat-tax table.data').addClass('empty-data');
      }
    }

    var lastNewRowId = -1;

    jQuery('.edit-vat-tax button.new-rate').click(
      function () {
        var base = jQuery('.edit-vat-tax tr.new-template');
    ...
    jQuery('.edit-vat-tax td.rate button.rate-remove').click(
      function () {
        ...
        if (!isNaN(id)) {
          if (id > 0) {
            ...
            var page = jQuery(this).parents('form').get(0).elements.namedItem('page').value;
            core.post(
              URLHandler.buildURL({target: 'taxes', action: 'VATTaxRemoveRate', page: page, id: id}),
              function(XMLHttpRequest, textStatus, data, valid) {
