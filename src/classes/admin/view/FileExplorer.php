...
* @package Dialog
* @access public
* @version $Id: FileExplorer.php,v 1.9 2008/10/23 11:46:56 sheriff Exp $
*/
class CFileExplorer extends CColumnList
{
    ...
    function getData()
    {
        ...
        for ($i=0; $i<count($children); $i++) {
            $children[$i] = func_new("FileNode", $path . '/'. $children[$i]);
        }
        if (preg_match('/^(.*\/.*\/.*)\/[^\/]+$/', $path, $matches)) {
            $par_node = func_new("FileNode",$matches[1]);
            $par_node->name = '..';
