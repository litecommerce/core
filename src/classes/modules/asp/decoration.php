...
/*
* $Id: decoration.php,v 1.9 2008/11/24 11:58:13 vgv Exp $
*/
...
function &func_new($class) // {{{
{
    ...
    if (class_exists($class)) {
        ...
        $class = func_define_class(strtolower($class), $primaryInstallation.'/classes/');
        if (!class_exists($class)) {
    ...
    if (method_exists($result,'getInstanceByClass')) {
        ...
        func_define_class(strtolower($className), $primaryInstallation.'/classes/');
...
function func_is_a($child, $parent) { // {{{
    ...
    func_define_class($child, $primaryInstallation.'/classes/');
...
function &func_get_instance($class, $param = null) // {{{
{
    ...
    if (!isset($instances[$class.':'.$param])) {
        $dclass = func_define_class($class, $primaryInstallation.'/classes/');
    ...
    if (!isset($xlite_defined_classes[$originalClass])) {
        ...
        if (isset($xlite_class_files_state[$originalClass])) {
            ...
            if (!file_exists($compiledFile) || filemtime($file) != filemtime($compiledFile)) {
                ...
                mkdirRecursive(dirname($compiledFile), get_filesystem_permissions(0755));
                if($cfp = fopen($compiledFile, 'wb')) {
                    fwrite($cfp, $source);
                    fclose($cfp);
                    @chmod($compiledFile, get_filesystem_permissions(0644));
...
<?php
        ...
        if (\$this->get("template") == "common/dialog.tpl" && (isset(\$_REQUEST["gid"]) || isset(\$_REQUEST["GID"]) || isset(\$_REQUEST["gmid"]) || isset(\$_REQUEST["GMID"]))) {
			...
			\$license_data["build_version"] = "2.2.39";
