...
* @access public
* @version $Id: Product.php,v 1.9 2008/12/05 15:02:15 vgv Exp $
*/
class Module_Egoods_Product extends Product
{
	...
	{
		return $this->get('pinSettings.pin_type');
	}
	...
	function createLink($file_id)
	{
		...
		$dl->set('exp_time', mktime(0, 0, 0, 
				date("n", time()), 
				date("j", time()) + $this->get('xlite.config.Egoods.exp_days'), 
				date("Y", time())
		));
		
		$dl->set('available_downloads', $this->get('xlite.config.Egoods.exp_downloads'));
		$dl->set('expire_on', $this->get('xlite.config.Egoods.link_expires'));
		$dl->set('link_type', 'A');
		$dl->create();
