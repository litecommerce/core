...
* @package ProtxForm
* @access public
* @version $Id: install.php,v 1.3 2008/05/22 08:33:07 vgv Exp $
*/
...
start_patching("ProtxForm");

$location = "skins/default/en/checkout/details_dialog.tpl";

if (!is_template_patched($location, "ProtxForm"))
{
	$find_str = <<<EOT
...
EOT;
    $replace_str = <<<EOT
<widget template="checkout/credit_card.tpl" visible="{cart.paymentMethod.formTemplate=#checkout/credit_card.tpl#}">
<widget template="modules/ProtxForm/checkout.tpl"  visible="{cart.paymentMethod.formTemplate=#modules/ProtxForm/checkout.tpl#}" />
EOT;
    patch_template($location, "ProtxForm", $find_str, $replace_str);
}
else
{
