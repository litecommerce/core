...
* @access public
* @version $Id: Bulletin.php,v 1.7 2009/08/13 19:40:14 isden Exp $
*/
class CBulletin extends Component
{
    ...
    function &getLastNews()
    {
        if (is_null($this->lastNews)) {
            ...
            $this->lastNews = array();
            if (is_array($nls)) {
            	for ($i=$minNL; $i <= $maxNL; $i++) {
            		$this->lastNews[] = $nls[$i];
            		if (count($this->lastNews) == $recent_messages) {
            			break;
            		}
            	}
            }
