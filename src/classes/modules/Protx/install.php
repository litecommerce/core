...
* @package Protx
* @access public
* @version $Id: install.php,v 1.2 2008/05/22 08:33:07 vgv Exp $
*/
...
start_patching("Protx");

$location = "skins/default/en/checkout/details_dialog.tpl";

if (!is_template_patched($location, "Protx"))
{
	$find_str = <<<EOT
...
EOT;
    $replace_str = <<<EOT
<widget template="checkout/credit_card.tpl" visible="{cart.paymentMethod.formTemplate=#checkout/credit_card.tpl#}">
<widget template="modules/Protx/checkout.tpl"  visible="{cart.paymentMethod.formTemplate=#modules/Protx/checkout.tpl#}" />
EOT;
    patch_template($location, "Protx", $find_str, $replace_str);
}
else
{
