...
start_patching("PayPalPro");

$location = "skins/default/en/checkout/details_dialog.tpl";

if (!is_template_patched($location, "PayPalPro"))
{
	$find_str = <<<EOT
<!-- PAYMENT METHOD FORM -->
EOT;
...
<!-- PAYMENT METHOD FORM -->
EOT;
    patch_template($location, "PayPalPro", $find_str, $replace_str);
}
else
{
...
<widget module="PayPalPro" target="checkout" mode="register" template="common/dialog.tpl" body="modules/PayPalPro/retrieve_profile.tpl" head="Make checkout easier with PayPal Website Pro" visible="{xlite.PayPalProExpressEnabled}">
EOT;
    patch_template($location, $replace_str, $find_str, $replace_str);
}
