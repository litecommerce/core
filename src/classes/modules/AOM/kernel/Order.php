...
* @package AOM
* @access public
* @version $Id: Order.php,v 1.50 2008/10/23 11:49:41 sheriff Exp $
*/
class Module_AOM_Order extends Order
{
	...
	function orderStatusChanged()
	{
        ...
        if ($this->xlite->config->get("AOM.status_inheritance")) {
		...
		if ($dc->get("type") != "absolute") {
			...
			if ($this->get("config.Taxes.prices_include_tax") && $this->xlite->AOM_product_originalPrice) {
				...
				foreach ($this->get("items") as $i) {
					$price = $i->get("price");
					$p = func_new("Product", $i->get("product_id"));
					$p->set("price", $price);
    ...
    function calcTotal()
    {
		if ($this->xlite->AOM_skip_calcTotal)
			return;
