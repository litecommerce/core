...
* @package AOM
* @access public
* @version $Id: encoded.php,v 1.20 2008/10/31 08:29:20 vgv Exp $
*/
...
function aom_order_clone(&$_this, &$clone) 
{
	...
	if ($_this->xlite->get("mm.activeModules.Promotion")) {
		...
		$cart =& func_new("Cart", $clone->get("order_id"));
...
function aom_get_clone_order(&$_this)
{
	...
	if (is_null($_this->clone_order)) {
		...
		if ($found) {
			$_this->clone_order =& func_new("Order", $aom_orders[$_this->get("order_id")]);
		} else {
...
function aom_get_profile(&$_this)
{
    ...
    if (is_null($_this->profile)) {  
        ...
        if (is_null($_this->profile)) {
            $_this->profile =& func_new("Profile");
