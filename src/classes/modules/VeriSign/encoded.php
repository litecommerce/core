<?php
    function func_Verisign_process(&$lite_cart, &$paymentMethod, $debug = false)
    {
        // license check
        check_module_license("VeriSign");
        ...
        $libdir = $path."/classes/modules/VeriSign/lib";
        $bindir = $path."/classes/modules/VeriSign/bin";
        $certdir = "./var/tmp";
		if (!is_dir($certdir)) {
			mkdir($certdir);
		} 
...
#
# $Id: encoded.php,v 1.7 2006/07/26 13:57:46 osipov Exp $
#
...
// Live and test gateway addersses
$vs_gtw = $module_params["param05"];
$vs_test_gtw = $module_params["param06"];
// transaction type
$vs_trx_type = (isset($module_params["param07"])) ? $module_params["param07"] : "S";

$vs_host = ($module_params["testmode"] != "N") ? $vs_test_gtw : $vs_gtw;
...
putenv("LD_LIBRARY_PATH=".getenv("LD_LIBRARY_PATH").":".$libdir);
putenv("PFPRO_CERT_PATH=".$certdir);
exec($bindir."/pfpro $vs_host 443 " . escapeshellarg(join("&",$post)) . " 2>&1", $bill_output);
