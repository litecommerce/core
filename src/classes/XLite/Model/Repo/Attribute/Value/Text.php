 ...
 * @since 1.0.16
 */
class Text extends \XLite\Model\Repo\Attribute\Value
{
    /**
     ...
     * @param string                     $data         Attribute value to search
     *
    ...
    protected function getExpressionPartForProductIDsByValuesQuery(\Doctrine\ORM\QueryBuilder $queryBuilder, $id, $data)
    {
        ...
        if (!empty($data)) {
            ...
            $result->add('av.value LIKE :value' . $id);
            $queryBuilder->setParameter('value' . $id, '%' . strval($data) . '%');
        }
