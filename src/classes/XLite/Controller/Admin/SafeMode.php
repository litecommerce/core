<?php
// vim: set ts=4 sw=4 sts=4 et:

/**
 * LiteCommerce
 * 
 * NOTICE OF LICENSE
 * 
 * This source file is subject to the Open Software License (OSL 3.0)
 * that is bundled with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://opensource.org/licenses/osl-3.0.php
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to licensing@litecommerce.com so we can send you a copy immediately.
 * 
 * @category   LiteCommerce
 * @package    XLite
 * @subpackage Controller
 * @author     Creative Development LLC <info@cdev.ru> 
 * @copyright  Copyright (c) 2011 Creative Development LLC <info@cdev.ru>. All rights reserved
 * @license    http://opensource.org/licenses/osl-3.0.php Open Software License (OSL 3.0)
 * @version    GIT: $Id$
 * @link       http://www.litecommerce.com/
 * @see        ____file_see____
 * @since      3.0.0
 */

namespace XLite\Controller\Admin;

/**
 * ____description____
 * 
 * @package XLite
 * @see     ____class_see____
 * @since   3.0.0
 */
class SafeMode extends \XLite\Controller\Admin\AAdmin
{
    /**
     * Handles the request to admin interface
     *
     * @return void
     * @access public
     * @see    ____func_see____
     * @since  3.0.0
     */
    public function handleRequest()
    {
        if ('run' !== \XLite\Core\Request::getInstance()->action) {
            $this->setReturnURL($this->buildURL('main'));
        }

        parent::handleRequest();
    }

    /**
     * Run safe mode
     *
     * @return void
     * @access protected
     * @see    ____func_see____
     * @since  3.0.0
     */
    protected function doActionRun()
    {
        if ($this->config->General->safe_mode_key === \XLite\Core\Request::getInstance()->access_key) {

            $modules = \XLite\Core\Database::getRepo('\XLite\Model\Module')->getActiveModules();

            if ($modules) {

                foreach ($modules as $m) {
                    $m->disableModule();
                }

                \XLite::setCleanUpCacheFlag(true);
            }

        } else {

            \XLite\Logger::getInstance()->log(
                'Attempt to run safe mode with invalid key: '
                . \XLite\Core\Request::getInstance()->access_key,
                LOG_ERR
            );

        }

        // To avoid the infinite loop
        $this->setReturnURL($this->buildURL('main'));
    }

}
