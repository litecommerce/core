...
$replace_str = <<<EOT
            ...
            <td>
                <widget module="WishList" template="modules/CDev/WishList/add.tpl" href="javascript: if (isValid()) document.add_to_cart.target.value = 'wishlist'; document.add_to_cart.action.value = 'add'; document.add_to_cart.submit();">
            </td>
EOT;
...
EOT;
$replace_str = <<<EOT
                <widget module="WishList" template="modules/CDev/WishList/add.tpl" href="javascript: WishList_Add2Cart();">
EOT;
...
EOT;
    $replace_str = <<<EOT
                ...
                <tr>    
                    ...
                    <td width="40">&nbsp;</td> 
                    <td><widget module="WishList" template="modules/CDev/WishList/add.tpl" href="cart.php?target=wishlist&action=add&product_id={product.product_id}"></td>
                </tr>       
                </table> 
EOT;
    patch_template($location, "WishList", $find_str, $replace_str);
...
EOT;
    $replace_str = <<<EOT
<widget module="WishList" template="modules/CDev/WishList/location.tpl">
</font>
EOT;
    patch_template($location, "WishList", $find_str, $replace_str);
}
...
EOT;
    	$replace_str = <<<EOT
<widget module="WishList" target="wishlist,product" mode="MessageSent" template="common/dialog.tpl" body="modules/CDev/WishList/message.tpl" head="Message has been sent">
<widget module="WishList" target="wishlist" head="Wish List" template="common/dialog.tpl" body="modules/CDev/WishList/wishlist.tpl">
<widget module="WishList" target="product" head="Send to a friend" template="common/dialog.tpl" body="modules/CDev/WishList/send_to_friend.tpl">
<!-- [/center] -->
EOT;
...
EOT;
    $replace_str = <<<EOT
<widget module="WishList" target="wishlists" template="modules/CDev/WishList/wishlists.tpl" head="Wish Lists">
<widget module="WishList" target="wishlist" template="common/dialog.tpl" body="modules/CDev/WishList/wishlist.tpl" head="Wish List">
<!-- [/center] -->
EOT;
...
{
    $find_str = <<<EOT
<widget module="Promotion" template="modules/CDev/Promotion/location.tpl">
EOT;
    $replace_str = <<<EOT
<widget module="Promotion" template="modules/CDev/Promotion/location.tpl">
<widget module="WishList" template="modules/CDev/WishList/location.tpl">
...
EOT;
    $replace_str = <<<EOT
<a href="admin.php?target=order_list">Orders</a><br>
<widget module="WishList" template="modules/CDev/WishList/menu.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
else
...
EOT;
    	$replace_str = <<<EOT
<!-- [main] {{{ -->
<widget module="WishList" mode="wishlist" template="common/dialog.tpl" body="modules/CDev/WishList/wishlist_note.tpl" head="Wishlist Notification">
EOT;
    	patch_template($location, null, $find_str, $replace_str);
    }
    else
...
<span IF="xlite.WishListEnabled">
<widget module="WishList" template="modules/CDev/WishList/mini_cart/body.tpl">
</span>
EOT;
    ...
    if (!is_template_patched($location, "modules/CDev/WishList/add.tpl"))
    {
    	$find_str = <<<EOT
            <td colspan=2>
                <widget class="CButton" label="Add to Cart" href="javascript: if (isValid()) document.add_to_cart.submit()" img="cart4button.gif" font="FormButton">
