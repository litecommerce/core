...
start_patching('Affiliate');

$location = "skins/default/en/main.tpl";
    
if (!is_template_patched($location, "Affiliate"))
{
    $find_str = <<<EOT
...
<widget module="Affiliate" template="modules/CDev/Affiliate/location.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
} else {
    already_patched($location);
...
EOT;
    $replace_str = <<<EOT
{*extraFields*}
<widget module="Affiliate" template="modules/CDev/Affiliate/partner_fields.tpl" name="partnerProfileForm">
EOT;
    patch_template($location, null, $find_str, $replace_str);
} else {
    already_patched($location);
