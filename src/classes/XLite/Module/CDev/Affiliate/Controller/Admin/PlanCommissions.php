...
class PlanCommissions extends \XLite\Controller\Admin\AAdmin
{
    ...
    function action_update_commission()
    {
        if ($this->get('update')) {
            ...
            foreach ($this->get('commission') as $itemID => $commission) {
                $pc = new \XLite\Module\CDev\Affiliate\Model\PlanCommission($this->get('plan_id'), $itemID, $this->get('item_type'));
    ...
    {
        if (is_null($this->pc)) {
            $this->pc = new \XLite\Module\CDev\Affiliate\Model\PlanCommission();
        }
        return $this->pc;
    }
}
