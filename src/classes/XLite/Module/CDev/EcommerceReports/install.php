...
EOT;
    $replace_str = <<<EOT
<widget template="stats.tpl">
<widget module="EcommerceReports" template="modules/CDev/EcommerceReports/reports.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
} else {
    already_patched($location);
...
EOT;
    $replace_str = <<<EOT
<a href="admin.php?target=orders_stats">Statistics</a><br>
<widget module="EcommerceReports" template="modules/CDev/EcommerceReports/maintenance.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
} else {
    already_patched($location);
