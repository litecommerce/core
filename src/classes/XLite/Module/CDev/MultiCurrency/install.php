...
start_patching('MultiCurrency');

$location = "skins/admin/en/main.tpl";
if (!is_template_patched($location, "MultiCurrency"))
{
    $find_str = <<<EOT
...
if (!is_template_patched($location, "currencies"))
{
    $find_str = <<<EOT
...
EOT;
    $replace_str = <<<EOT
<widget module="Promotion" template="modules/CDev/Promotion/location.tpl">
<widget module="MultiCurrency" template="modules/CDev/MultiCurrency/location.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
else
