...
EOT;
    $replace_str = <<<EOT
<widget module="EcommerceReports" template="modules/CDev/EcommerceReports/location.tpl">
<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/location.tpl">
EOT;
...
EOT;
    $replace_str = <<<EOT
<widget module="Affiliate" template="modules/CDev/Affiliate/main.tpl">
<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/main.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
else
...
EOT;
    ...
    <tr><td align="right">&nbsp;<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/settings_link.tpl" IF="module.class=#ups#"/></td></tr>
EOT;
        patch_template($location, null, $find_str, $replace_str);
    }
    else
...
if (!file_exists($location)) {
    ...
    if (!preg_match('/document\.shipping_method_\{module.class\}.submit\(\)\;\"\>(?(?!\<\/tr\>).)*?\<\/tr\>/si', $content, $out)) {
        ...
        if (!is_template_patched($location, "UPSOnlineTools/settings_disclaimer.tpl"))
        {
            ...
            $replace_str = <<<EOT
$find_str
<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/settings_disclaimer.tpl" IF="module.class=#ups#"/>
EOT;
            patch_template($location, null, $find_str, $replace_str);
        } else {
            already_patched($location);
...
EOT;
    $replace_str = <<<EOT
<b><a href="admin.php?target=order&mode=invoice&order_id={order.order_id}" target="_blank"><input type="image" src="images/go.gif" width="13" height="13" border="0" align="absmiddle"> Print invoice</a></b>
<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/show_container_details.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
else
...
//------------------------------------------------
$location = "skins/admin/en/modules/CDev/AOM/order.tpl";
if (file_exists($location)) {
    if (!is_template_patched($location, "UPSOnlineTools"))
    {
    	$find_str = <<<EOT
...
EOT;
        $replace_str = <<<EOT
<input class="ProductDetailsTitle" type="button" value=" Print invoice " onClick="window.open('admin.php?target=order&mode=invoice&order_id={order.order_id}')">
            <widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/show_container_details.tpl" style="button">
EOT;
        patch_template($location, null, $find_str, $replace_str);
    } else {
        already_patched($location);
...
//------------------------------------------------
$location = "skins/default/en/main.tpl";
if (!is_template_patched($location, '<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/main.tpl">'))
{
...
<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/notice_register.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
else
...
EOT;
    	$replace_str = <<<EOT
<p IF="cart.shippingAvailable&cart.shipped&cart.getShippingRates()" align="right">
<widget module="UPSOnlineTools" template="modules/CDev/UPSOnlineTools/delivery.tpl">
...
//------------------------------------------------
$location = "skins/default/en/modules/CDev/Promotion/delivery.tpl";
if (file_exists($location)) {
    if (!is_template_patched($location, "UPSOnlineTools"))
    {
    	$find_str = <<<EOT
