...
class Product extends \XLite\Model\Product implements \XLite\Base\IDecorator
{
     ...
     * inventory 
     * 
     * @var    \XLite\Module\CDev\InventoryTracking\Model\Inventory
     * @access protected
     * @since  3.0.0
     */
    ...
    function getInventory()
    {
        if (is_null($this->inventory)) {
            $this->inventory = new \XLite\Module\CDev\InventoryTracking\Model\Inventory();
    ...
    function filter()
    {
        ...
        if (!$this->xlite->is('adminZone') && $this->config->CDev->InventoryTracking->exclude_product) {
    ...
    public function isInStock()
    {
        ...
        if ($max_options && $this->get('tracking')) {
            $inv = new \XLite\Module\CDev\InventoryTracking\Model\Inventory();
    ...
    function updateInventory($properties)
    {
        ...
        $option = new \XLite\Module\CDev\ProductOptions\Model\ProductOption($properties['option_id']);
        ...
        if ($properties['optclass']!=$old_properties['optclass'])    
            foreach ($inventories as $inventory_) {
                $inventory = new \XLite\Module\CDev\InventoryTracking\Model\Inventory($inventory_->getComplex('properties.inventory_id'));
                $inventory->delete();
    ...
    function cloneInventory($product, $options = false)
    {
        ...
        {
            $inventories = new \XLite\Module\CDev\InventoryTracking\Model\Inventory();
    ...
    function cloneObject()
    {
        ...
        $p->set('tracking', 0);
        $p->update();
        if ($this->config->CDev->InventoryTracking->clone_inventory) {
    ...
    function createDefaultInventory()
    {
        ...
        if (!$this->xlite->get('ITisCloneProduct') && $this->config->CDev->InventoryTracking->create_inventory) {
            $inventory = new \XLite\Module\CDev\InventoryTracking\Model\Inventory();
            $inventory->set('inventory_id',$this->get('product_id'));
            $inventory->set('amount',$this->config->CDev->InventoryTracking->inventory_amount);
            $inventory->set('low_avail_limit',$this->config->CDev->InventoryTracking->low_amount);
    ...
    function collectGarbage()
    {
        ...
        if (is_array($result) && count($result) > 0) {
            foreach ($result as $info) {
                $pi = new \XLite\Module\CDev\InventoryTracking\Model\Inventory($info['inventory_id']);
                $pi->delete();
            }
        }
    }
