...
namespace XLite\Module\CDev\GiftCertificates\Validator;
...
class GCValidator extends \XLite\Validator\AValidator
{
     ...
     * @since  3.0.0
     */
    public $template = 'modules/CDev/GiftCertificates/gc_validator.tpl';
    ...
    public function isValid()
    {
        ...
        if (!parent::isValid()) {
            ...
            if (!is_object($cart) || is_null($cart) || $cart->get('gcid') != $this->gcid) {
                ...
                // validate
                $gc = new \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate($this->gcid);
                $gcStatus = 0 == strlen($this->gcid)
                    ? \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate::GC_DOESNOTEXIST
                    : $gc->validate();

                $result = false;

                switch ($gcStatus) {
                    case \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate::GC_OK: 
                        $result = true;
                        break;

                    case \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate::GC_DOESNOTEXIST: 
                        $this->doesnotexist = true;
                        break;

                    case \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate::GC_EXPIRED: 
                        $this->expired = true;
                        break;

                    case \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate::GC_DISABLED: 
                        $this->notactive = true;
                        break;
