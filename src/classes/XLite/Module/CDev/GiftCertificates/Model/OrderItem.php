...
class OrderItem extends \XLite\Model\OrderItem implements \XLite\Base\IDecorator
{
    ...
    public function getGC()
    {
        if (is_null($this->gc)) {
            ...
            $this->gc = $gcId ? new \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate($gcId) : null;
        }
    ...
    {
        if (is_null($gc) || !($gc instanceof \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate)) {
            $this->gc = null;
            $this->set('gcid', '');
    ...
    {
        return (\XLite\Core\Database::getRepo('\XLite\Model\Module')->isModuleActive('CDev\ProductOptions') && $this->getProduct())
            ? parent::hasOptions()
            : false;
    }
