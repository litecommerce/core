...
class Order extends \XLite\Model\Order implements \XLite\Base\IDecorator
{
    ...
    function orderStatusChanged()
    {
        ...
        if ($this->xlite->config->AOM->status_inheritance) {
            ...
            $substatus = new \XLite\Module\CDev\AOM\Model\OrderStatus();
    ...
    function getAppliedGC() 
    {
        $gcid = $this->get('gcid');
        if (!empty($gcid)) {
            $gc = new \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate($gcid);
        }
        return $gc;
    }
 	...
 	function getOrderGC()  
    {
        ...
        foreach ($items as $key => $item) {
            ...
            if (empty($gcid)) 
                ...
                $gc[$item->get('item_id')] = new \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate($gcid);
        }
    ...
    function _getAppliedGlobalDiscount()
    {
        ...
        if (!is_object($gd)) $gd = new \XLite\Module\CDev\WholesaleTrading\Model\GlobalDiscount();
    ...
    function _getAppliedDiscount()
    {
        ...
        $dc = $this->get('orderDC');
        if (!is_object($dc)) $dc = new \XLite\Module\CDev\Promotion\Model\DiscountCoupon();
    ...
    function getOrderStatus()  
    {
        ...
        $this->orderStatus = new \XLite\Module\CDev\AOM\Model\OrderStatus();
