...
EOT;
    $replace_str = <<<EOT
    <td>Active</td>
    <widget module="AntiFraud" template="modules/CDev/AntiFraud/risk_country/label.tpl">
EOT;
...
EOT;
    ...
    <widget module="AntiFraud" template="modules/CDev/AntiFraud/risk_country/checkbox.tpl">
</tr>
EOT;
...
EOT;
    $replace_str = <<<EOT
<widget module="AntiFraud" target='order' mode="{mode}" template="common/dialog.tpl" body="modules/CDev/AntiFraud/tracking/message.tpl" head="Antifraud Service Notification" visible="{mode}">
<widget module="AntiFraud" target='order' template="common/dialog.tpl" body="modules/CDev/AntiFraud/order.tpl" head="Antifraud Service" visible="{order.details.af_result}">
<widget target="order_list" template="order/search.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
...
EOT;
    $replace_str = <<<EOT
    <td align=left>Status</td>
    <widget module="AntiFraud" template="modules/CDev/AntiFraud/orders/label.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
...
EOT;
    $replace_str = <<<EOT
    <td><widget template="common/order_status.tpl"></td>
    <widget module="AntiFraud" template="modules/CDev/AntiFraud/orders/factor.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
else
...
<widget module="AntiFraud" template="modules/CDev/AntiFraud/orders/search_form.tpl">
EOT;
    patch_template($location, null, $find_str, $replace_str);
}
else
