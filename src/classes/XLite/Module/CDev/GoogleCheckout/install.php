...
EOT;
    $replace_str = <<<EOT
{*extraFields*}
<widget module="GoogleCheckout" template="modules/CDev/GoogleCheckout/product/info.tpl">
EOT;
...
EOT;
    $replace_str = <<<EOT
{*extraFields*}
<widget module="GoogleCheckout" template="modules/CDev/GoogleCheckout/product/add.tpl">
EOT;
    patch_template($location, $check_str, $find_str, $replace_str);
} else {
    already_patched($location);
...
<widget target="order" template="common/dialog.tpl" body="order/order.tpl" head="Order # {order.order_id}">
{else:}
<widget module="AOM" template="modules/CDev/AOM/main.tpl">
{end:}
EOT;
    patch_template($location, $check_str, $find_str, $replace_str);
} else {
...
$location = "skins/admin/en/main.tpl";
$check_str = "modules/CDev/GoogleCheckout/main.tpl";
if (!is_template_patched($location, $check_str)) {
    $find_str = <<<EOT
...
EOT;
    $replace_str = <<<EOT
<widget target="shipping_zones" class="CTabber" body="{pageTemplate}" switch="target">
<widget module="GoogleCheckout" template="modules/CDev/GoogleCheckout/main.tpl">
EOT;
    patch_template($location, $check_str, $find_str, $replace_str);
} else {
    already_patched($location);
...
$location = "skins/admin/en/modules/CDev/AOM/common/select_status.tpl";
...
EOT;
    $replace_str = <<<EOT
<widget module="GoogleCheckout" template="modules/CDev/GoogleCheckout/google_checkout.tpl" visible="{cart.paymentMethod.payment_method=#google_checkout#}">
<!-- PAYMENT METHOD FORM -->
EOT;
...
EOT;
        $replace_str = <<<EOT
<widget class="CButton" label="Delete item" href="cart.php?target=cart&action=delete&cart_id={cart_id}" font="FormButton">
        <widget module="GoogleCheckout" template="modules/CDev/GoogleCheckout/shopping_cart/item.tpl">
EOT;
        patch_template($location, "GoogleCheckout", $find_str, $replace_str);
    } else {
        already_patched($location);
...
$location = "skins/default/en/modules/CDev/Promotion/item.tpl";
if (file_exists($location)) {
...
$location = "skins/default/en/main.tpl";
$check_str = 'modules/CDev/GoogleCheckout/google_checkout_dialog.tpl';
if (!is_template_patched($location, $check_str))
{
    $find_str = <<<EOT
<!-- [/checkout] }}} -->
EOT;
    if (is_template_patched($location, $find_str)) {
    	$replace_str = <<<EOT
<widget module="GoogleCheckout" template="common/dialog.tpl" body="modules/CDev/GoogleCheckout/google_checkout_dialog.tpl" head="Google Checkout payment module" visible="{target=#googlecheckout#&!valid}" >
<!-- [/checkout] }}} -->
EOT;
    	patch_template($location, $check_str, $find_str, $replace_str);
    } else {
...
$mod = @file_get_contents('skins/default/en/modules/CDev/GoogleCheckout/button_alt_checkout.tpl');
if ($button && $mod && preg_match("/\{\* ORIGINAL BUTTON TEMPLATE \*\}.*\{\* \/ORIGINAL BUTTON TEMPLATE \*\}/s", $mod)) {
    ...
    if ($handle = fopen('skins/default/en/modules/CDev/GoogleCheckout/button_alt_checkout.tpl', "w")) {
        fwrite($handle, $out);
        fclose($handle);
    }
}
...
$check_str = 'GoogleCheckout';
if (is_template_patched($location, '<widget template="modules/CDev/GoogleCheckout/shopping_cart/gcheckout_notes.tpl">')) {
    patch_template($location, null, '<widget template="modules/CDev/GoogleCheckout/shopping_cart/gcheckout_notes.tpl">', '<widget module="GoogleCheckout" template="modules/GoogleCheckout/shopping_cart/gcheckout_notes.tpl">');
} else {
    if (!is_template_patched($location, $check_str)) {
        $find_str = <<<EOT
...
EOT;
        $replace_str = <<<EOT
<widget template="shopping_cart/buttons.tpl">
<widget module="GoogleCheckout" template="modules/CDev/GoogleCheckout/shopping_cart/gcheckout_notes.tpl">
EOT;
        patch_template($location, $check_str, $find_str, $replace_str);
    } else {
        already_patched($location);
