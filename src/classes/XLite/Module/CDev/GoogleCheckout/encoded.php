...
function GoogleCheckout_getXML_Object()
{
    ...
    return new \XLite\Module\CDev\GoogleCheckout\Model\XML();
}
...
//			$_this->DC = null;
        ...
        foreach ($discounts as $id=>$discount) {
            ...
            $coupon = new \XLite\Module\CDev\Promotion\Model\DiscountCoupon();
...
EOT;
    ...
    if ($_this->xlite->get('GiftCertificatesEnabled') && is_array($discounts) && count($discounts) > 0) {
        ...
        foreach ($discounts as $id=>$discount) {
            ...
            $cert = new \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate();
            if (!$cert->find("gcid='".addslashes($cert_code)."'"))
                continue;

            $discounts[$id]['type'] = "cert";

            if ($cert->validate() != \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate::GC_OK || $cert->get('debit') <= 0)
                continue;
...
function GoogleCheckout_new_order_notification($_this, $xmlData)
{
    ...
    if ($_this->xlite->get('GiftCertificatesEnabled') && $gift_cert) {
        ...
        if ($result == \XLite\Module\CDev\GiftCertificates\Model\GiftCertificate::GC_OK) {
